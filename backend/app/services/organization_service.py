...
class OrganizationService:
    ...
    async def get_organization_tree(self, db: AsyncSession, user: User) -> list[dict]:
        ...
        try:
            ...
            if user.is_superuser:
                ...
                stmt = select(Organization).where(not Organization.is_private).options(selectinload(Organization.children)).order_by(Organization.level, Organization.sort_order)
    ...
    async def update_organization(self, db: AsyncSession, org_id: int, data: dict[str, Any]) -> Organization | None:
    ...
    async def get_organization_by_name(self, db: AsyncSession, name: str) -> Organization | None:
        """根据名称获取组织"""
        try:
